   ...
   let playerName: UILabel = {
    ...
    func animateStats() {
        UIView.animate(withDuration: 1.0) {
            ...
            self.boarder.alpha = 0.5
            self.cancel.alpha = 1
            self.age.layer.position.x += 60
            self.country.layer.position.x += 60
            self.boarder.layer.position.y -= 100
        }
    }
    ...
    func accelerate() {
        UIView.animate(withDuration: 0.25) {
            self.status.layer.position.x += 120
            self.playerName.layer.position.x += 120
        }
    }
    
    
    func animateMeta() {
        self.status.layer.position.y += 320
        self.playerName.layer.position.y += 320
